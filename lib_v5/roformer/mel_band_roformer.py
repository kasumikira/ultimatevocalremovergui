...
def log_weighted_mae(
    ...
    """
    Computes the Log Weighted Mean Absolute Error between reconstructed and target magnitudes.
    """
    if weight is None:
        weight = torch.ones_like(target_mag)

    mae = torch.abs(recon_mag - target_mag)
    weighted_mae = mae * weight
    mean_weighted_mae = torch.mean(weighted_mae)
    log_wmae = torch.log(mean_weighted_mae + eps)
    return log_wmae
...
def l2_freq_loss(
    ...
    """
    Computes the L2 Frequency Loss (Mean Squared Error) between reconstructed and target magnitudes.
    """
    mse = F.mse_loss(recon_mag, target_mag)
    return mse
...
class MelBandRoformer(Module):
    ...
    def __init__(
        ...
        # ensure the edge frequencies are covered without giving them artificial full weight
